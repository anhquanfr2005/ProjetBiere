...
<?php
	...
	?>
		<div id ="boi" class="navbar navbar-expand-xl navbar-dark fixed-top bg-dark">
          <div class="container-xl">
            ...
            <a href="" class="d-flex align-items-center"><img style="width: 100px;" src="./imge/LOGO-TAVERN.png" alt="logo"></a>
		...
		?>
                <li class="nav-item">
                  <a style="color: greenyellow;" href="insertion.php?action=inserer_utilisateur" title="Insérer un utilisateur">Ajouter des bières</a></li>
				  <?php
		}
		if (basename($_SERVER['PHP_SELF']) != 'suppression.php') {
		?>
				  <li class="nav-item">
				  <a style="color: red;" href="suppression.php?action=supprimer_utilisateur" title="Supprimer un utilisateur">Supprimer le stock</a></li>	
					<?php
		}
		if (basename($_SERVER['PHP_SELF']) != 'modification.php') {
		?>
                <li class="nav-item">
				<a style="color: rgb(0, 162, 255);" href="modification.php?action=modifier_utilisateur" title="Modifier un utilisateur">Modifier le stock</a></li>	
		<?php
		}
	}
		?>  
	...
	function FormulaireModifStock(){
		...
		<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
			<fieldset> 
				...
				<button type="button" value='3' onclick="prerempl(this);">Récupérer les imformations de la bière</button>
				<br>
				<label for="id_quant">Nombre de bière dans le stock : </label><input id="moi" type="number" name="Quantite" id="id_quant" autofocus><br>
	...
	function afficheindex(){
		if ($_SESSION['statut'] == "admin"){
			...
			echo '<h2 class="textes">Ces 5 bière ci dessous sont les plus vendu et risque d\'être en rupture de stock</h2>';
		} else {
			echo '<h1 class="textes">Accueil</h1>';
			echo '<h2 class="textes">Top 5 de nos bière les plus vendu</h2>';
		}
	}
	function captcha(){
		?> 
